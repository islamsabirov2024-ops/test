...
"""CREATE TABLE IF NOT EXISTS ads(id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id INTEGER, title TEXT, text TEXT, start_enabled INTEGER DEFAULT 0, movie_enabled INTEGER DEFAULT 0, views INTEGER DEFAULT 0, active INTEGER DEFAULT 1, scheduled_at INTEGER DEFAULT 0, sent INTEGER DEFAULT 0, created_at INTEGER)""",
...
async def take_balance(user_id:int, amount:int):
    async with conn() as db:
        ...
        if bal < int(amount): return False
...
async def add_movie(bot_id:int, code:str, title:str='', caption:str='', premium:int=0):
    now=int(time.time())
    async with conn() as db:
        cur=await db.execute('INSERT OR REPLACE INTO movies(bot_id,code,title,caption,premium,created_at,updated_at) VALUES(?,?,?,?,?,?,?)',(bot_id,code.lower().strip(),title,caption,int(premium),now,now))
        await db.commit()
        if cur.lastrowid: return cur.lastrowid
        r=await (await db.execute('SELECT id FROM movies WHERE bot_id=? AND code=?',(bot_id,code.lower().strip()))).fetchone(); return r['id']
...
async def add_ad(bot_id:int,title,text,scheduled_at=0):
    async with conn() as db:
        await db.execute('INSERT INTO ads(bot_id,title,text,scheduled_at,created_at) VALUES(?,?,?,?,?)',(bot_id,title,text,int(scheduled_at or 0),int(time.time()))); await db.commit()
