...
"""CREATE TABLE IF NOT EXISTS it_lessons(id INTEGER PRIMARY KEY AUTOINCREMENT,user_bot_id INTEGER NOT NULL,category TEXT DEFAULT 'python',title TEXT NOT NULL,body TEXT DEFAULT '',image_file_id TEXT DEFAULT '',price INTEGER DEFAULT 0,is_active INTEGER DEFAULT 1,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
"""CREATE TABLE IF NOT EXISTS lesson_purchases(id INTEGER PRIMARY KEY AUTOINCREMENT,user_bot_id INTEGER NOT NULL,lesson_id INTEGER NOT NULL,user_id INTEGER NOT NULL,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,UNIQUE(user_bot_id,lesson_id,user_id))""",
"""CREATE TABLE IF NOT EXISTS download_logs(id INTEGER PRIMARY KEY AUTOINCREMENT,user_bot_id INTEGER,user_id INTEGER,url TEXT,title TEXT DEFAULT '',created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"""
]
async def init_db():
...
# ===== IT LESSONS =====
async def add_lesson(bid:int, category:str, title:str, body:str, image_file_id:str='', price:int=0):
    await execq('INSERT INTO it_lessons(user_bot_id,category,title,body,image_file_id,price,is_active) VALUES(?,?,?,?,?,?,1)',bid,category,title,body,image_file_id,int(price))
...
async def get_lesson(bid:int,lid:int): return await row('SELECT * FROM it_lessons WHERE user_bot_id=? AND id=? AND is_active=1',bid,lid)
async def delete_lesson(bid:int,lid:int): await execq('UPDATE it_lessons SET is_active=0 WHERE user_bot_id=? AND id=?',bid,lid)
async def buy_lesson(bid:int,lid:int,uid:int): await execq('INSERT OR IGNORE INTO lesson_purchases(user_bot_id,lesson_id,user_id) VALUES(?,?,?)',bid,lid,uid)
async def has_lesson(bid:int,lid:int,uid:int): return await row('SELECT 1 FROM lesson_purchases WHERE user_bot_id=? AND lesson_id=? AND user_id=?',bid,lid,uid) is not None
async def seed_default_lessons(bid:int):
    existing=await rows('SELECT id FROM it_lessons WHERE user_bot_id=? LIMIT 1',bid)
    if existing: return
    defaults=[
        ('python','1-dars: Python nima?','Python — bot, web va avtomatlashtirish uchun eng oson til.\n\nKod: print("Salom")',0),
        ('python','2-dars: O‘zgaruvchilar','name="Islam"\nage=18\nprint(name, age)',5000),
        ('bot','1-dars: Telegram bot nima?','BotFather orqali token olinadi. Keyin Python aiogram bilan ulaymiz.',0),
        ('bot','2-dars: /start komandasi','Router, Dispatcher va handler tushunchasi. Pullik dars namunasi.',10000),
        ('web','1-dars: HTML boshlanishi','HTML sahifa skeleti: <html><body>Salom</body></html>',0),
        ('db','1-dars: Database nima?','Ma’lumotlar bazasi user, kino, to‘lovlarni saqlaydi.',0),
        ('deploy','1-dars: Render/Railway','Deploy uchun requirements.txt, start command va env kerak.',0),
    ]
    for cat,title,body,price in defaults:
        await add_lesson(bid,cat,title,body,'',price)
