...
async def set_movie_premium(bot_id:int, code:str, is_premium:int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE movies SET is_premium=? WHERE user_bot_id=? AND code=?", (int(is_premium), bot_id, code.lower().strip()))
        await db.commit()

async def increment_movie_views(bot_id:int, code:str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE movies SET views=COALESCE(views,0)+1 WHERE user_bot_id=? AND code=?", (bot_id, code.lower().strip()))
        await db.commit()
