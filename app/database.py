...
# ===== PRO qo'shimcha helperlar =====
async def run_migrations():
    async with connect() as db:
        db.row_factory = aiosqlite.Row
        for sql in [
            "ALTER TABLE subscribers ADD COLUMN premium_until INTEGER DEFAULT 0",
            "ALTER TABLE subscribers ADD COLUMN ref_balance INTEGER DEFAULT 0",
            "ALTER TABLE bots ADD COLUMN site_url TEXT DEFAULT ''",
            "ALTER TABLE bots ADD COLUMN support_text TEXT DEFAULT ''",
        ]:
            try:
                await db.execute(sql)
            except Exception:
                pass
        await db.execute("CREATE TABLE IF NOT EXISTS premium_tariffs(id INTEGER PRIMARY KEY AUTOINCREMENT, bot_id INTEGER, name TEXT, days INTEGER, price INTEGER, is_active INTEGER DEFAULT 1, created_at INTEGER)")
        await db.commit()
...
async def list_subscribers(bot_id:int, limit:int=50):
    async with connect() as db:
        db.row_factory = aiosqlite.Row
        cur=await db.execute("SELECT * FROM subscribers WHERE bot_id=? ORDER BY created_at DESC LIMIT ?",(bot_id,limit))
        return [dict(r) for r in await cur.fetchall()]

async def count_subscribers(bot_id:int):
    async with connect() as db:
        db.row_factory = aiosqlite.Row
        cur=await db.execute("SELECT COUNT(*) c FROM subscribers WHERE bot_id=?",(bot_id,)); return (await cur.fetchone())['c']

async def set_premium(bot_id:int,user_id:int,until:int):
    async with connect() as db:
        db.row_factory = aiosqlite.Row
        await db.execute("INSERT OR IGNORE INTO subscribers(bot_id,user_id,username,full_name,created_at,premium_until) VALUES(?,?,?,?,?,?)",(bot_id,user_id,'','',int(time.time()),until))
        await db.execute("UPDATE subscribers SET premium_until=? WHERE bot_id=? AND user_id=?",(until,bot_id,user_id))
        await db.commit()

async def list_premium_users(bot_id:int):
    now=int(time.time())
    async with connect() as db:
        db.row_factory = aiosqlite.Row
        cur=await db.execute("SELECT * FROM subscribers WHERE bot_id=? AND premium_until>? ORDER BY premium_until DESC",(bot_id,now))
        return [dict(r) for r in await cur.fetchall()]

async def add_premium_tariff(bot_id:int,name:str,days:int,price:int):
    async with connect() as db:
        db.row_factory = aiosqlite.Row
        await db.execute("INSERT INTO premium_tariffs(bot_id,name,days,price,created_at) VALUES(?,?,?,?,?)",(bot_id,name,days,price,int(time.time())))
        await db.commit()

async def list_premium_tariffs(bot_id:int):
    async with connect() as db:
        db.row_factory = aiosqlite.Row
        cur=await db.execute("SELECT * FROM premium_tariffs WHERE bot_id=? AND is_active=1 ORDER BY price ASC",(bot_id,))
        return [dict(r) for r in await cur.fetchall()]

async def delete_bot(bot_id:int):
    async with connect() as db:
        db.row_factory = aiosqlite.Row
        await db.execute("DELETE FROM bots WHERE id=?",(bot_id,)); await db.commit()
