...
def admin_menu():
    # KINO BOT ADMIN PANELI — foydalanuvchi yuborgan 2-rasmdagi dizayn
    return reply([
...
def text_settings_menu():
    return inline([
        ...
        [('🔐 Obuna matni', 'text:subscribe_text')],
        [('💎 Premium matni', 'text:premium_text')],
        [('💳 To‘lov matni', 'text:payment_text')],
        [('🔙 Orqaga', 'admin:home')],
    ])


def sub_settings_menu(force='0', fake='0'):
    return inline([
        [(f'🔐 Majburiy obuna: {"✅ ON" if force=="1" else "❌ OFF"}', 'set:toggle:force_sub_enabled')],
        [(f'🧪 Hiyla/Fake verify: {"✅ ON" if fake=="1" else "❌ OFF"}', 'set:toggle:sub_fake_verify')],
        [('🔙 Orqaga', 'admin:home')],
    ])
