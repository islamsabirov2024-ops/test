...
# ===== TARIFLAR =====
TARIFFS = {
    "start": {"title": "🚀 Start", "price": 18000, "days": 30, "users": "0-1000"},
    "standard": {"title": "⚡ Standard", "price": 35000, "days": 30, "users": "1000-5000"},
    "pro": {"title": "💎 Pro", "price": 90000, "days": 30, "users": "5000-20000"},
    "vip": {"title": "👑 VIP", "price": 200000, "days": 30, "users": "VIP"},
}
...
def main_menu(is_admin: bool = False):
    ...
    if is_admin:
        rows.append(
            [KeyboardButton(text="🧾 To‘lovlar"), KeyboardButton(text="📊 Admin statistika")]
        )
...
def bot_types():
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="👥 Invite bot", callback_data="type:invite_gate")],
...
def tariff_kb(bot_id: int):
    rows = [
        [
            InlineKeyboardButton(
                text=f"{v['title']} — {money(v['price'])} so‘m",
                callback_data=f"tariff:{bot_id}:{k}",
            )
        ]
...
def manage_bot(bot_id: int, active: bool):
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="▶️ Yoqish", callback_data=f"run:{bot_id}"),
                InlineKeyboardButton(text="⏸ O‘chirish", callback_data=f"stop:{bot_id}"),
            ],
            ...
            [
                InlineKeyboardButton(text="💳 Tarif/To‘lov", callback_data=f"tariffs:{bot_id}"),
                InlineKeyboardButton(text="🗑 O‘chirish", callback_data=f"delete:{bot_id}"),
            ],
        ]
    )
...
def movie_admin_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="📺 Kino kanal"), KeyboardButton(text="🔐 Majburiy obuna")],
            [KeyboardButton(text="⭐ Premium"), KeyboardButton(text="📊 Statistika")],
...
def movie_add_kb():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🎬 Kinolar"), KeyboardButton(text="📺 Kino kanal")],
...
def cleaner_settings_kb(s: dict):
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text=f"{mark('links')} Ssilka", callback_data="cl:t:links"),
                InlineKeyboardButton(text=f"{mark('mentions')} @kanal", callback_data="cl:t:mentions"),
            ],
            [
                InlineKeyboardButton(text=f"{mark('forward')} Forward", callback_data="cl:t:forward"),
                InlineKeyboardButton(text=f"{mark('buttons')} URL tugma", callback_data="cl:t:buttons"),
            ],
            [
                InlineKeyboardButton(text=f"{mark('words')} Spam so‘z", callback_data="cl:t:words"),
                InlineKeyboardButton(text=f"{mark('ban')} Warn/Ban", callback_data="cl:t:ban"),
            ],
        ]
    )
...
def it_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🚀 Deploy"), KeyboardButton(text="🧠 Pro maslahat")],
        ],
        resize_keyboard=True,
    )
