...
def _is_user_premium(user: dict | None) -> bool:
    return bool(user and int(user.get("is_premium", 0) or 0) == 1)
...
def build_subscribe_kb(channels: list[dict], promos: list[dict]) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton("💎 Premium", callback_data="buy_menu")])
...
async def send_subscribe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "💎 <b>Premium obuna</b> sotib olib, kanallarga obuna bo'lmasdan foydalanishingiz mumkin."
    )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_subs":
        ...
        if pending_code:
            ...
            if movie.get("is_premium") and not _is_user_premium(user):
                await query.message.reply_text("🔒 <b>Ushbu kino faqat premium foydalanuvchilar uchun</b>\n\n❗️ Kinoni ko'rish uchun premium obuna sotib oling.", parse_mode="HTML", reply_markup=premium_buy_kb())
                return
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.isdigit():
        ...
        if not movie:
            ...
            return
        # HAR DOIM yangidan tekshiramiz — admin yangi kanal qo'shsa, darhol talab qilinadi
        passed = await _real_subscription_passed(update, context)
        if not passed:
        ...
        if movie.get("is_premium") and not _is_user_premium(user):
            await message.reply_text("🔒 <b>Ushbu kino faqat premium foydalanuvchilar uchun</b>\n\n❗️ Kinoni ko'rish uchun premium obuna sotib oling.", parse_mode="HTML", reply_markup=premium_buy_kb())
            return
        await _send_movie(update, context, movie)
        return
    await message.reply_text("❌ Noma'lum buyruq yoki noto'g'ri kod.")
