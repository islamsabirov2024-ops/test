...
async def create_bot_token(m:Message, state:FSMContext):
    ...
    try:
        ...
        await db.set_setting(bot_id,'referral_bonus','0'); await db.set_setting(bot_id,'sub_fake_verify','0'); await db.set_setting(bot_id,'premium_enabled','1'); await db.set_setting(bot_id,'text_Premium_kino_xabari','🔒 Bu kino premium. Ko‘rish uchun 💎 Premium olish tugmasini bosing.'); await db.set_setting(bot_id,'antispam_enabled','1'); await db.set_setting(bot_id,'antispam_limit','5'); await db.set_setting(bot_id,'antispam_window','3'); await db.set_setting(bot_id,'antispam_block','10')
