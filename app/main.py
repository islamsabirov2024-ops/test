...
async def choose_kino_bot(m:Message, state:FSMContext):
    ...
    await m.answer(
        ...
        "qidiruv, serial/qism, TOP, sevimlilar va kino so‘rovlar mavjud.\n\n"
...
async def create_bot_token(m:Message, state:FSMContext):
    ...
    try:
        ...
        await db.set_setting(bot_id,'premium_enabled','1'); await db.set_setting(bot_id,'antispam_enabled','1'); await db.set_setting(bot_id,'antispam_limit','5'); await db.set_setting(bot_id,'antispam_window','3'); await db.set_setting(bot_id,'antispam_block','10')
...
async def bot_toggle(c:CallbackQuery):
    ...
    new='paused' if r['status']=='active' else 'active'; await db.set_bot_status(bot_id,new)
...
async def add_movie_2(m:Message, state:FSMContext):
    await state.update_data(code=m.text.strip()); await state.set_state(AddMovie.title); await m.answer('🎬 Kino nomini yuboring:')
@child_router.message(AddMovie.title)
async def add_movie_3(m:Message, state:FSMContext):
    await state.update_data(title=m.text.strip()); await state.set_state(AddMovie.premium); await m.answer('💎 Bu kino premium bo‘ladimi?\n1 — Ha\n0 — Yo‘q')
@child_router.message(AddMovie.premium)
async def add_movie_4(m:Message, state:FSMContext):
    await state.update_data(premium=1 if m.text.strip().lower() in {'1','ha','yes'} else 0, part_no=1); await state.set_state(AddMovie.media)
    await m.answer('📥 1-qism videosini/documentini yuboring. Keyin yana qism qo‘shishni so‘rayman.')
...
async def add_movie_5(m:Message, state:FSMContext):
    ...
    mv=await db.get_movie(bid,data['code'])
    movie_id=mv['id'] if mv else await db.add_movie(bid,data['code'],data.get('title',''),cap,data.get('premium',0))
    part_no=int(data.get('part_no',1)); await db.add_part(movie_id,part_no,f'{part_no}-qism',file_id=file_id,caption=cap)
    await state.update_data(movie_id=movie_id, part_no=part_no+1); await state.set_state(AddMovie.more)
    await m.answer(f"✅ {part_no}-qism saqlandi.\n\nYana qism qo‘shasizmi?\n1 — Ha\n0 — Yo‘q")
@child_router.message(AddMovie.more)
async def add_movie_more(m:Message, state:FSMContext):
    if m.text.strip() in {'1','ha','Ha','HA'}:
        d=await state.get_data(); await state.set_state(AddMovie.media); return await m.answer(f"📥 {d.get('part_no',2)}-qism videosini/documentini yuboring:")
    d=await state.get_data(); await log_action(await runtime_db_id(m.bot.id),m.from_user.id,'kino_qoshdi',d.get('code',''))
    await state.clear(); await m.answer('✅ Kino/serial to‘liq saqlandi', reply_markup=content_menu())
...
async def edit_m1(m:Message, state:FSMContext):
    if not await is_child_admin(m): return
    await state.set_state(EditMovie.code); await m.answer('✏️ Premium holatini o‘zgartirish uchun kino kodini yuboring:')
@child_router.message(EditMovie.code)
async def edit_m2(m:Message, state:FSMContext):
    bid=await runtime_db_id(m.bot.id); new=await db.toggle_movie_premium(bid,m.text.strip()); await log_action(bid,m.from_user.id,'kino_tahrirladi',m.text.strip()); await state.clear()
    await m.answer('❌ Kino topilmadi' if new is None else f"✅ Premium holati: {'YOQILDI 💎' if new else 'O‘CHIRILDI'}", reply_markup=content_menu())

# CHANNELS
@child_router.message(F.text=='🔐 Kanallar')
async def ch_menu(m:Message):
    if not await is_child_admin(m): return
    await m.answer('🔐 Majburiy obuna kanallari:', reply_markup=channels_menu())
@child_router.message(F.text=='➕ Kanal qo‘shish')
async def add_ch1(m:Message,state:FSMContext):
    if not await is_child_admin(m): return
    await state.set_state(AddChannel.title); await m.answer('📢 Kanal nomini kiriting:')
@child_router.message(AddChannel.title)
async def add_ch2(m:Message,state:FSMContext): await state.update_data(title=m.text.strip()); await state.set_state(AddChannel.chat); await m.answer('🆔 Kanal ID yoki username kiriting (@kanal yoki -100...)')
@child_router.message(AddChannel.chat)
async def add_ch3(m:Message,state:FSMContext): await state.update_data(chat=m.text.strip()); await state.set_state(AddChannel.url); await m.answer('🔗 Kanal havolasini kiriting:')
@child_router.message(AddChannel.url)
async def add_ch4(m:Message,state:FSMContext):
    d=await state.get_data(); await db.add_channel(await runtime_db_id(m.bot.id),d['title'],d['chat'],m.text.strip(),1); await state.clear(); await m.answer('✅ Kanal qo‘shildi', reply_markup=channels_menu())
@child_router.message(F.text=='📋 Ro‘yxatni ko‘rish')
async def ch_list(m:Message):
    rows=await db.channels(await runtime_db_id(m.bot.id)); await m.answer('📋 Kanallar:\n'+'\n'.join([f"{r['id']}. {r['title']} — {r['chat_id']} | o‘tgan: {r['pass_count']}" for r in rows]) if rows else 'Kanal yo‘q')
...
async def pay_panel(m:Message): await m.answer('⚙️ To‘lov tizim sozlamalaridasiz:', reply_markup=pay_menu())
...
async def pay_add3(m:Message,state:FSMContext):
    d=await state.get_data(); await db.add_pay_method(await runtime_db_id(m.bot.id),d['name'],m.text); await state.clear(); await m.answer('✅ To‘lov tizimi qo‘shildi', reply_markup=pay_menu())
...
async def tariff_panel(m:Message):
    ...
    await m.answer(txt, reply_markup=rkb([['➕ Tarif qo‘shish'],['◀️ Asosiy panel']]))
...
@child_router.message(F.text=='💎 Premium')
...
async def search1(m:Message,state:FSMContext): await state.set_state(SearchMovie.query); await m.answer('🔍 Kino nomi yoki kodini yozing:')
...
async def broadcast2(m:Message,state:FSMContext):
    await state.update_data(text=m.text); await state.set_state(Broadcast.confirm); await m.answer('📢 Preview:\n\n'+m.text+'\n\nYuborilsinmi? 1 — Ha / 0 — Yo‘q')
@child_router.message(Broadcast.confirm)
async def broadcast3(m:Message,state:FSMContext):
    if m.text.strip()!='1': await state.clear(); return await m.answer('Bekor qilindi')
    d=await state.get_data(); await state.clear(); await m.answer('✅ Broadcast navbatga qo‘yildi. (Oddiy DB users bo‘yicha yuborish katta botlarda sekin bo‘ladi.)')
...
async def send_movie(m:Message, mv):
    bid=await runtime_db_id(m.bot.id); parts=await db.parts(mv['id'])
    if not parts: return await m.answer('❌ Kino fayli topilmadi.')
    await db.inc_view(bid,mv['id'],m.from_user.id)
    title=await db.get_setting(bid,'text_Kino_qismlari_sarlavhasi','🎬 Kino qismlari')
    if len(parts)>1: await m.answer(f"{title}: {mv['title']}\nJami qismlar: {len(parts)}")
    for p in parts:
        cap=p['caption'] or mv['caption'] or await db.get_setting(bid,'text_Kino_caption_matni','')
        if len(parts)>1: cap=(f"🎬 {mv['title']}\n📌 {p['name'] or str(p['part_no'])+'-qism'}\n\n"+cap).strip()
        ad_movie=await db.get_setting(bid,'ad_movie','0')
        if ad_movie=='1':
            ads=await db.ads(bid)
            if ads: cap+=(('\n\n' if cap else '')+'📢 '+ads[0]['text'])
        if p['file_id']: await m.answer_video(p['file_id'], caption=cap, protect_content=True, reply_markup=movie_inline(mv['id']))

@child_router.message(F.text)
async def movie_code(m:Message):
    if m.text in {'🎬 Kino kodi yuborish'}: return await m.answer('🎬 Kino kodini yozib yuboring:')
    bid=await runtime_db_id(m.bot.id)
    ...
    if mv['premium'] and not await db.has_premium(bid,m.from_user.id): return await m.answer(await db.get_setting(bid,'text_Premium_kino_xabari','🔒 Bu kino premium. Ko‘rish uchun 💎 Premium bo‘limidan tarif sotib oling.'))
