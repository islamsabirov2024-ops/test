...
async def choose_kino_bot(m:Message, state:FSMContext):
    ...
    await m.answer(
        ...
        "qidiruv, TOP, sevimlilar, premium, reklama va kino so‘rovlar mavjud.\n\n"
...
async def add_movie_4(m:Message, state:FSMContext):
    await state.update_data(premium=1 if m.text.strip().lower() in {'1','ha','yes'} else 0)
    await state.set_state(AddMovie.media)
    await m.answer('📥 Kino videosini yoki documentini yuboring.')
...
async def send_movie(m:Message, mv):
    ...
    txt='🎬 Kino qismlari ro‘yxati:\n\n' + '\n'.join([f"{p['part_no']}. {p['part_no']}-qism" for p in rows])
    await m.answer(txt, reply_markup=movie_parts_inline(rows))
...
async def movie_part_callback(c:CallbackQuery):
    ...
    if mv['premium'] and not await db.has_premium(bid,c.from_user.id):
        return await c.message.answer(await db.get_setting(bid,'text_Premium_kino_xabari','🔒 Bu kino premium. Ko‘rish uchun 💎 Premium bo‘limidan tarif sotib oling.'))
