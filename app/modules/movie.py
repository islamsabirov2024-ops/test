...
from app.keyboards.common import movie_admin, movie_admin_reply, cancel_menu, movie_channel_menu, movie_section_menu, movie_add_method_menu
...
def setup(dp: Dispatcher, bot_id: int, owner_id: int):
    ...
    async def admin_home(m: Message):
        await m.answer('👮 <b>Admin panel</b>', reply_markup=movie_admin_reply())
        await m.answer(
    ...
    async def title(m: Message, state: FSMContext):
        ...
        await m.answer(
            ...
            reply_markup=movie_add_method_menu(),
        )

    @r.callback_query(F.data.startswith('m:addinfo:'))
    async def add_info(c: CallbackQuery):
        if c.from_user.id != owner_id:
            await c.answer('Ruxsat yo‘q', show_alert=True); return
        mode = (c.data or '').split(':')[-1]
        if mode == 'forward':
            txt = '📨 Kanal postini shu botga forward qiling. Bot o‘sha kanalda admin bo‘lsa userga copy qilib beradi.'
        elif mode == 'link':
            txt = '🔗 Public link: <code>https://t.me/kanal/123</code> yoki private: <code>https://t.me/c/123456/123</code> yuboring.'
        elif mode == 'file':
            txt = '📁 Video yoki documentni to‘g‘ridan-to‘g‘ri shu botga yuboring.'
        else:
            txt = '🌐 Tashqi ssilka hozir eslatma uchun. Eng ishonchlisi: forward, post link yoki video/document.'
        await c.message.answer(txt, reply_markup=movie_add_method_menu())
        await c.answer()
    ...
    async def owner_text_menu(m: Message, state: FSMContext):
        ...
        if text == '📊 Statistika':
            ...
            return
        if text == '🎬 Kinolar':
            await m.answer('🎬 <b>Kinolar bo‘limi</b>\n\n<blockquote>Kerakli amalni tanlang.</blockquote>', reply_markup=movie_section_menu())
            return
        if text == '🔐 Kanallar':
            settings = await get_settings(bot_id)
            chs = settings.get('force_channels', []) or []
