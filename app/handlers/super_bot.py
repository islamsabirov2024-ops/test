...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_super_admin(user.id):
        ...
        if text in ('🎬 Kontent boshqaruvi', '🔐 Kanallar', '⚙️ Tizim sozlamalari'):
            await update.message.reply_text('✅ Bu bo‘lim child Kino Bot ichida to‘liq ishlaydi. Botlar ro‘yxatidan kerakli botni ishga tushiring.', reply_markup=super_main()); return
        if text == '⏪ Orqaga':
            await update.message.reply_text('👑 Super Admin Panel', reply_markup=super_main()); return
...
async def callback(update, context):
    ...
    if data.startswith('sbot:'):
        ...
        if status == 'delete':
            await db.set_bot_status(int(bot_id), 'deleted')
        else:
