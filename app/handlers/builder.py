...
from app.db import (
    ...
    set_bot_tariff, add_bot_days, count_user_bots, get_settings, save_settings,
)
...
async def got_name(m: Message, state: FSMContext):
    ...
    run_msg = 'Bot saqlandi, lekin avtomatik ishga tushmadi.'
    try:
        b = await get_bot(bot_id)
        if manager and b:
            ok, run_msg = await manager.start_child(dict(b))
            if ok:
                await set_bot_status(bot_id, 'active')
    except Exception as e:
        run_msg = 'Avto start xatosi: ' + str(e)[:120]
    await state.clear()
    await m.answer(
        '✅ <b>Bot yaratildi!</b>\n\n'
        ...
        f'📆 Muddati: 30 kun\n\n'
        f'<blockquote>{run_msg}\n\n“🤖 Botlarim” bo‘limidan sozlamalarni boshqaring.</blockquote>',
...
@router.callback_query(F.data.startswith('clearcache:'))
async def clear_cache(c: CallbackQuery):
    bot_id=int(c.data.split(':')[1]); b=await get_bot(bot_id)
    if not b or b['owner_user_id'] != c.from_user.id:
    ...
    await c.message.answer('🧹 <b>Kesh tozalandi.</b>\n\n<blockquote>Bot vaqtinchalik holatlari yangilandi. Fayllar serverga saqlanmagani uchun kino/video o‘chmaydi.</blockquote>', reply_markup=settings_bot_kb(bot_id, b['status']=='active'))
    await c.answer()

@router.callback_query(F.data.startswith(('dashboard:', 'newtoken:', 'transfer:', 'adminid:')))
