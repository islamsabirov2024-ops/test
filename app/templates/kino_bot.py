...
from app.keyboards.kino import (
    ...
    settings_menu, premium_menu, payments_menu, ads_menu, text_settings_menu, subscribe_kb
)
...
async def text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if admin:
        if t in ('⏪ Orqaga', '⬅️ Orqaga', 'Asosiy panel'):
            await update.message.reply_text('👑 Admin panel', reply_markup=admin_menu()); return
        if t == '📊 Statistika':
        ...
        if t in ('📥 So‘rovlar', '👥 Foydalanuvchilar', '📮 Postlar', '🔗 Referal', '↗️ Ulashish'):
            await update.message.reply_text('✅ Bu bo‘lim tayyor. Keyingi yangilashda kengaytiriladi.', reply_markup=admin_menu()); return
    ...
    if t == '💎 Premium':
        await update.message.reply_text('💎 Premium bo‘limi tez orada to‘liq ishga tushadi.', reply_markup=user_menu()); return
...
async def cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'premium:mark':
        await q.message.reply_text('💎 Premium berish keyingi versiyada kengaytiriladi. Hozir Premium holatini yoqib/o‘chirishingiz mumkin.'); return
    if data in ('premium:users', 'premium:tariffs'):
        await q.message.reply_text('💎 Premium ro‘yxat/tarif moduli tayyor menyuda bor, keyingi yangilashda to‘liq CRUD qilinadi.'); return
