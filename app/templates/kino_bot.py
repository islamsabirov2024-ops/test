...
from app.keyboards.kino import admin_menu, user_menu, movies_menu, channels_menu, payments_menu, premium_menu, ads_menu, settings_menu, subscribe_kb
...
async def text(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if await is_admin(update, context):
        if t == '📊 Statistika':
            s=await db.stats(bot_id); await update.message.reply_text(f"📊 Statistika\n\n👥 Users: {s['users']}\n🎬 Kinolar: {s['movies']}\n🔐 Kanallar: {s['channels']}\n👁 Ko‘rishlar: {s['views']}"); return
        if t == '🎬 Kinolar': await update.message.reply_text('🎬 Kontent boshqaruvi', reply_markup=movies_menu()); return
        if t == '🔐 Kanallar': await show_channels_menu(update, context); return
        if t == '💳 To‘lov tizimlari': await update.message.reply_text('💳 To‘lov sozlamalari', reply_markup=payments_menu()); return
        if t == '💎 Premium': await update.message.reply_text('💎 Premium sozlamalari', reply_markup=premium_menu(await db.get_setting(bot_id,'premium_enabled','0'))); return
        ...
        if t == '⚙️ Sozlamalar': await update.message.reply_text('⚙️ Tizim sozlamalari', reply_markup=settings_menu()); return
    if is_menu_text(t):
    ...
    if movie.get('file_id'):
        await update.message.reply_video(movie['file_id'], caption=f"🎬 {movie.get('title') or movie['code']}")
...
async def handle_state(update, context, state, t):
    ...
    if state == 'movie_file':
        ...
        if link:
            ...
            context.user_data.clear(); await update.message.reply_text('✅ Kino link orqali saqlandi.'); return
        await update.message.reply_text('🎥 Video yuboring yoki to‘g‘ri link yuboring.'); return
    if state == 'delete_movie':
        await db.delete_movie(bot_id, clean_code(t)); context.user_data.clear(); await update.message.reply_text('🗑 Kino o‘chirildi.'); return
    if state == 'channel_add':
        parts=t.split('|')
        title=parts[0].strip() if parts else 'Kanal'; link=parts[1].strip() if len(parts)>1 else t.strip(); chat_id=parts[2].strip() if len(parts)>2 else link.replace('https://t.me/','@').replace('t.me/','@')
        await db.add_channel(bot_id,title,link,chat_id,1); context.user_data.clear(); await update.message.reply_text('✅ Kanal qo‘shildi.'); return
    if state == 'channel_delete':
        try: await db.delete_channel(bot_id,int(t))
        except Exception: pass
        context.user_data.clear(); await update.message.reply_text('🗑 Kanal o‘chirildi.'); return
    if state == 'card':
    ...
    if state.startswith('text:'):
        await db.set_setting(bot_id,state.split(':',1)[1],t); context.user_data.clear(); await update.message.reply_text('✅ Matn saqlandi.'); return
    if state == 'ads_text':
    ...
    if state == 'premium_mark':
        ...
        if movie:
            await save_movie(bot_id, movie['code'], movie['title'], movie['file_id'], movie['source_chat_id'], movie['source_message_id'], 1)
...
async def video(update, context):
    ...
    context.user_data.clear(); await update.message.reply_text('✅ Kino video orqali saqlandi.')

async def show_channels_menu(update, context):
    bot_id=context.application.bot_data['bot_id']
    await update.message.reply_text('🔐 Kanallar boshqaruvi', reply_markup=channels_menu(await db.get_setting(bot_id,'force_sub_enabled','0')))
...
async def cb(update, context):
    ...
    if data == 'movie:add': context.user_data['state']='movie_code'; await q.message.reply_text('🔢 Kino kodini yuboring:'); return
    if data == 'movie:list':
        movies=await db.list_movies(bot_id,30); txt='🎬 Kinolar ro‘yxati:\n\n'+'\n'.join([f"{m['id']}. {m['code']} — {m.get('title') or '-'} 👁 {m['views']}" for m in movies]) if movies else 'Kinolar yo‘q.'; await q.message.reply_text(txt); return
    if data == 'movie:delete': context.user_data['state']='delete_movie'; await q.message.reply_text('🗑 O‘chirish uchun kino kodini yuboring:'); return
    if data == 'channel:add': context.user_data['state']='channel_add'; await q.message.reply_text('➕ Kanal yuboring:\nFormat: Nomi | https://t.me/kanal | @kanal'); return
    if data == 'channel:list':
        chs=await db.list_channels(bot_id); await q.message.reply_text('🔐 Kanallar:\n\n'+'\n'.join([f"{c['id']}. {c['title']} — {c['chat_id']}" for c in chs]) if chs else 'Kanal yo‘q.'); return
    if data == 'channel:delete': context.user_data['state']='channel_delete'; await q.message.reply_text('🗑 Kanal ID yuboring:'); return
